MY OWN MEMORY MANAGER by Toni Lopez (UPC-MVJ)

- To support me in developing, I will use the following reference links:
	
	- 	http://www.codeproject.com/Articles/15527/C-Memory-Pool
	-	http://www.cantrip.org/wave12.html
	-	http://www.ibm.com/developerworks/linux/library/l-memory/
	-	http://www.ibm.com/developerworks/aix/tutorials/au-memorymanager/

- I will do a develpment in steps using de MVP model

1 - Simple memory allocator: 1 pool & 1 data size